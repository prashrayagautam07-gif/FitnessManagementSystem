...
// Get current data
$stmt = $conn->prepare("SELECT name, email, phone, membership_type FROM members WHERE member_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("UPDATE members SET name = ?, email = ?, phone = ?, membership_type = ? WHERE member_id = ?");
